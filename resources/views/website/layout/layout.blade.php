...
<head>
    ...
    <meta content="Free HTML Templates" name="{{$setting->translate(app()->getlocale())->title}}">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <div class="owl-carousel owl-carousel-1 tranding-carousel position-relative d-inline-flex align-items-center ml-3" style="width: calc(100% - 100px); padding-left: 90px;">
                        @foreach ($lastFivePosts as $post)
                            <div class="text-truncate"><a class="text-secondary" href="{{Route('post.show',$post->id)}}">{{ $post->title }}</a></div>
                        @endforeach
                    </div>
                </div>
            </div>
                        ...
                        @if (count($category->children) > 0)
                            <div class="dropdown-menu rounded-0 m-0">
                                @foreach ($category->children as $child)
                                    <a href="{{ route('category.show', $child->id) }}" class="dropdown-item">{{ $child->title }}</a>
                                @endforeach
                            </div>
                        @endif
                    </div>
    ...
    <div class="container-fluid bg-light pt-5 px-sm-3 px-md-5">
        <div class="row">
            ...
            <div class="col-lg-6 col-md-6 mb-5">
                ...
                @foreach ($categories as $category)
                <a href="{{route('category.show',$category->id)}}" class="dropdown-item">{{ $category->title }}</a>
                @endforeach
                </div>
            </div>
